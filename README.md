...
- 管理页网址：`http://<服务器IP>:8080/sources`（员工界面无任何入口，请自行收藏）
- 打开时浏览器会弹出账号密码框（Basic 认证）：
  - 账号：`admin`
  - 密码：见 `config/admin.local.yaml` 的 `password`（该文件不入代码库，只存在本机）
- 修改密码：编辑 `config/admin.local.yaml` 后重启网站服务
- 忘记密码：直接打开该文件即可查看或重设
- 关闭密码（不建议，尤其公网部署后）：把该文件里 `enabled` 改成 `false`
- 管理页内含：各源抓取状态、立即抓取、最近日志下载、立即备份、用户反馈列表
- 员工提交反馈不受影响（公开可用）；每条反馈会同时发到邮箱（见第 4 节反馈邮件通知）
